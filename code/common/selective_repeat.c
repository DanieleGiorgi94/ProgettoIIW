...
static char sorted_buf_insertion(struct circular_buffer *, struct buf_node,
                                                                        u64);
static void send_ack(int, struct sockaddr, u64, char);
...
static void *receive_ack(void *arg) {
    ...
    while(1) {
        printf("attendo ack\n");
        if (recvfrom(sockfd, (void *) ack, sizeof(ack_t), 0, servaddr,
                                                        &slen) < 0) {
...
static void *selective_repeat_sender(void *arg) {
    ...
    int sockfd = ptd->send_sockfd;
    const struct sockaddr *servaddr = ptd->send_servaddr;

    for(;;) {
        printf("inizio for sr_sender\n");
        lock_buffer(cb);
...
void send_file(int send_sockfd, int rcv_sockfd, struct sockaddr *send_servaddr,
        struct sockaddr *rcv_servaddr, int fd, char *filename) {
    ...
    SR_td.send_sockfd = send_sockfd;
    ackrec.sockfd = rcv_sockfd;
    tmr_td.sockfd = send_sockfd;

    SR_td.send_servaddr = send_servaddr;
    ackrec.servaddr = rcv_servaddr;
    tmr_td.servaddr = send_servaddr;
...
//            pthread_join(sender_td.tid, NULL) != 0) {
    ...
    if (pthread_kill(tmr_td.tid, SIGTERM) == EINVAL) {
        free_allocation(cb);
        perror("Errore in pthread_kill()!");
        exit(EXIT_FAILURE);
    }
    free_allocation(cb);
...
static void *selective_repeat_receiver(void *arg) {
    struct SR_thread_data *ptd = arg;
    struct circular_buffer *cb = ptd->cb;
    int send_sockfd = ptd->send_sockfd;
    int rcv_sockfd = ptd->rcv_sockfd;
    const struct sockaddr *send_servaddr = ptd->send_servaddr;
    const struct sockaddr *rcv_servaddr = ptd->rcv_servaddr;
    unsigned int slen = sizeof(struct sockaddr);
    u64 seqnum;
    u32 nE;
    pkt_t *pkt;
    ...
    for(;;){
    	...
    	if (recvfrom(send_sockfd, (void *) pkt, sizeof(pkt_t), 0,
                (struct sockaddr *) send_servaddr, &slen) < 0) {
        ...
        if (sorted_buf_insertion(cb, cbn, seqnum)){
            // invio ack
            unlock_buffer(cb);
            send_ack(rcv_sockfd, *rcv_servaddr, seqnum, pkt_header.type);
            lock_buffer(cb);
        }
        unlock_buffer(cb);
    }
...
void receive_file(int send_sockfd, int rcv_sockfd,
        struct sockaddr *send_servaddr, struct sockaddr *rcv_servaddr, int fd) {
    ...
    receiver_td.cb = cb;
    SR_td.send_sockfd = send_sockfd;
    SR_td.rcv_sockfd = rcv_sockfd;
    SR_td.send_servaddr = send_servaddr;
    SR_td.rcv_servaddr = rcv_servaddr;
    receiver_td.fd = fd;
    
    if ((pthread_create(&receiver_td.tid, NULL, merge_file, &receiver_td) ||
        pthread_create(&SR_td.tid, NULL, selective_repeat_receiver,
                                                            &SR_td)) != 0) {
        free_allocation(cb);
        perror("Errore in pthread_create()");
        exit(EXIT_FAILURE);

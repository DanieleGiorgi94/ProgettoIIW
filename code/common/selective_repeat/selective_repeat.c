...
static void lock_socket(pthread_mutex_t *mtx)
{
...
static void unlock_socket(pthread_mutex_t *mtx)
{
...
//
//    pthread_mutex_t *mtx = ackrec->mtx;
//
//    while(1) {
//        printf("attendo ack\n");
//        lock_socket(mtx);
//        while (recvfrom(sockfd, (void *) ack, sizeof(ack_t), 0, servaddr,
//                                                        &slen) < 0) {
//            unlock_socket(mtx);
//            if (errno != EAGAIN) {
...
static void *receive_ack(int sockfd, struct sockaddr *servaddr,
...
static void *selective_repeat_sender(void *arg) {
    ...
    pthread_mutex_t *mtx = ptd->mtx;

    for(;;) {
        lock_buffer(cb);

        while (cb->S == cb->E) {
            printf("aspetto buffer\n");
        ...
        if (cb->N + BUFFER_SIZE * (cb->S > cb->N) - cb->S <= WINDOW_SIZE) {
            ...
            receive_ack(sockfd, servaddr, cb);
        }
        unlock_buffer(cb);
    }
    return NULL;
